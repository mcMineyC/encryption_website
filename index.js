...
function pi_code (inputElmnt, outputElmnt, PiDir) { // Set PiDir to 1 for encryption, -1 for decryption.
    ...
    const message = inputElmnt.value;
    let output = '';
    let PiI = 0;
    let pi_digit;
...
function base_compress () {
    const message = unencElmnt.value;
...
function base_decompress () {
    ...
    outBase = meMatch[1]
        ...
        .replace(/\\{/g, '{')
        .replace(/\\}/g, '}');
    let output = convert_base(meMatch[2], bases['stripped qwerty'], outBase);
    unencElmnt.value = outBase.at(-1) + output;
};
